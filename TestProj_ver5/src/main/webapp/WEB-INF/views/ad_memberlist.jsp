...
<head>
  ...
  $(document).ready(function(e){
		 $('#tb #tr').on('click', function() {
		     ...
		     var bn = '<input type="hidden" value="' + bon + '" name="boardName">';
		     $form.append(memId);
		     $form.append(bn);
		     $form.submit();
		}); 
		...
		$('#memDel').click( function() {
			...
			var bn = $('#boardName').text();
			var $form = $('#formValue');
			$form.attr('action', '/ad_memberlist/delete');
			var bon = '<input type="hidden" value="' + bn + '" name="boardName">';
		    $form.append(bon);
			 ...
			 <div style="font-weight:bold;">${boardName}회원 목록</div>
			  ...
			  <br>
			 	<div class="search">
					...
					<input type="text" class="form-control col-md-4" name="keyword" style="width:200px" placeholder="검색어 입력"/>
					...
					<ul class="pagination">
						...
						<c:forEach begin="${pageMaker.startPage }"
							...
							<li
								...
								<a href="/ad_memberlist/board${pageMaker.makeQuery(idx)}&boardName=${boardName}">${idx}</a>
							</li>
						</c:forEach>
