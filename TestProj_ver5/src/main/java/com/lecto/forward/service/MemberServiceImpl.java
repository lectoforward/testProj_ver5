...
public class MemberServiceImpl implements MemberService{
	...
	public Object[] searchManager(String searchWay, String keyword, Criteria cri) {
		...
		try {
			switch(searchWay){
			case "아이디":
				list.add(managerViewMapper.searchMemberId(keyword, cri));
				break;
			case "이름":
				for(Object o:managerViewMapper.searchMemberName(keyword, cri)){
					list.add(o);
				}
				break;
			case "게시판명":
				list.add(managerViewMapper.searchBoardName(keyword, cri));
				break;
			}
		} catch(Exception e) {
			e.printStackTrace();
	...
	public Object[] searchManager(Criteria cri) {
		Object[] arr = null;
		System.out.println("멤버서비스 서치매니저");
		List<ManagerVO> list = null;
		
		try {
			list = managerViewMapper.searchManagerViewAll(cri);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean isManager(String memberId){
		...
		try {
			...
			vo = managerViewMapper.searchMemberId(memberId, cri);
		} catch (Exception e) {
			e.printStackTrace();
		}
